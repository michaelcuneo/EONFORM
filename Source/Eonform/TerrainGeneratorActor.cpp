...
void ATerrainGeneratorActor::BuildTerrain()
{
	...
	if (bEnableHydraulicErosion && HydraulicIterations > 0)
	{
		...
		{
			HeightField.GetEonformField() = HydraulicErosionResult.Height;
	...
	FEonformTerrainDataset Dataset = FTerrainDatasetBridge::Build(
		HeightField,
		&TerrainContext,
		&ProcessMasks,
		&GeologyMaps,
	...
	FEonformTerrainDatasetMetadata DatasetMetadata;
	DatasetMetadata.HeightScale = HeightScale;
	FEonformTerrainDatasetRegistry::Publish(
