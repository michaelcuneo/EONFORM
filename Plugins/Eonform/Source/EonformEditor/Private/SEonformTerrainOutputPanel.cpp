...
void SEonformTerrainOutputPanel::InitializePresets()
{
	...
	static constexpr int32 Values[] = {0, 127, 253, 505, 1009, 2017, 4033};
...
FText SEonformTerrainOutputPanel::GetSectionComplexityLabel(EEonformTerrainOutputComplexity Complexity) const
{
	...
	case EEonformTerrainOutputComplexity::Responsive:
		return FText::FromString(TEXT("Responsive (~32K tris / region)"));
	case EEonformTerrainOutputComplexity::Detailed:
		return FText::FromString(TEXT("Detailed (~524K tris / region)"));
	case EEonformTerrainOutputComplexity::Maximum:
		return FText::FromString(TEXT("Maximum (~2M tris / region)"));
	case EEonformTerrainOutputComplexity::Balanced:
	default:
		return FText::FromString(TEXT("Balanced (~131K tris / region)"));
	}
}
...
TSharedPtr<EEonformTerrainOutputSectionLayout> SEonformTerrainOutputPanel::FindSectionLayoutPreset(EEonformTerrainOutputSectionLayout Layout) const
{
	...
	{
		if (Preset.IsValid() && *Preset == Layout)
			return Preset;
	}
...
TSharedPtr<EEonformTerrainOutputComplexity> SEonformTerrainOutputPanel::FindSectionComplexityPreset(EEonformTerrainOutputComplexity Complexity) const
{
	...
	{
		if (Preset.IsValid() && *Preset == Complexity)
			return Preset;
	}
...
FIntPoint SEonformTerrainOutputPanel::ResolveOutputResolution() const
{
	...
	{
		const int32 Resolution = FMath::Clamp(State.OutputResolution, 17, 4097);
...
FEonformMeshTerrainOutputSettings SEonformTerrainOutputPanel::MakeMeshTerrainSettings() const
{
	...
	Settings.TargetResolution = State.OutputResolution > 0
																	? FIntPoint(FMath::Clamp(State.OutputResolution, 17, 4097), FMath::Clamp(State.OutputResolution, 17, 4097))
																	: FIntPoint::ZeroValue;
...
bool SEonformTerrainOutputPanel::GenerateAvailableTerrain()
{
	...
	if (!FEonformTerrainDatasetRegistry::Get(TEXT("EonformGraph"), Snapshot) || !Snapshot.IsValid() || Snapshot.Revision != State.GetPublishedAnalysisRevision())
	{
		return false;
	}
	...
	ChildSlot
			[SNew(SBorder)
					 .Padding(FMargin(8.0f, 6.0f))
							 [SNew(SVerticalBox) + SVerticalBox::Slot().AutoHeight().Padding(0.0f, 0.0f, 0.0f, 8.0f)[SNew(STextBlock).Text(FText::FromString(TEXT("Terrain Output")))]

								+ SVerticalBox::Slot().AutoHeight().Padding(0.0f, 2.0f)
											[SNew(SObjectPropertyEntryBox)
													 .AllowedClass(FEonformMeshTerrainOutput::GetMeshPartitionDefinitionClass())
													 .ObjectPath_Lambda([]()
																							{ return FEonformTerrainOutputEditorState::Get().GetSettings().MeshPartitionDefinition.ToString(); })
								...
								SVerticalBox::Slot().AutoHeight().Padding(0.0f, 2.0f)
										[SNew(SHorizontalBox) + SHorizontalBox::Slot().FillWidth(1.0f).Padding(0.0f, 0.0f, 4.0f, 0.0f)[SNew(SNumericEntryBox<double>).Label()[SNew(STextBlock).Text(FText::FromString(TEXT("Width km")))].Value_Lambda([]()
																																																																																																																	 { return FEonformTerrainOutputEditorState::Get().GetSettings().WorldWidthKilometers; })
																																																											 .MinValue(0.001)
																																																											 .OnValueChanged_Lambda([](double V)
																																																																							{ FEonformTerrainOutputEditorState::Get().SetWorldWidthKilometers(V); })] +
										 SHorizontalBox::Slot().FillWidth(1.0f).Padding(4.0f, 0.0f, 0.0f, 0.0f)
												 [SNew(SNumericEntryBox<double>)
															...
															.Value_Lambda([]()
																						{ return FEonformTerrainOutputEditorState::Get().GetSettings().WorldDepthKilometers; })
															.MinValue(0.001)
															.OnValueChanged_Lambda([](double V)
																										 { FEonformTerrainOutputEditorState::Get().SetWorldDepthKilometers(V); })]] +
								SVerticalBox::Slot().AutoHeight().Padding(0.0f, 2.0f)
										[SNew(SHorizontalBox) + SHorizontalBox::Slot().FillWidth(1.0f).Padding(0.0f, 0.0f, 4.0f, 0.0f)[SNew(SNumericEntryBox<double>).Label()[SNew(STextBlock).Text(FText::FromString(TEXT("Elevation m")))].Value_Lambda([]()
																																																																																																																			{ return FEonformTerrainOutputEditorState::Get().GetSettings().ElevationScaleMeters; })
																																																											 .MinValue(0.001)
																																																											 .OnValueChanged_Lambda([](double V)
																																																																							{ FEonformTerrainOutputEditorState::Get().SetElevationScaleMeters(V); })] +
										 SHorizontalBox::Slot().FillWidth(1.0f).Padding(4.0f, 0.0f, 0.0f, 0.0f).VAlign(VAlign_Center)[SNew(STextBlock).Text(FText::FromString(TEXT("Sea Level  0 m (fixed)")))]]
								...
								SVerticalBox::Slot().AutoHeight().Padding(0.0f, 2.0f)
										[SNew(SComboBox<TSharedPtr<int32>>)
												 ...
												 .OnGenerateWidget_Lambda([this](TSharedPtr<int32> Item)
																									{ return SNew(STextBlock).Text(GetResolutionLabel(Item.IsValid() ? *Item : 0)); })
												 .OnSelectionChanged_Lambda([](TSharedPtr<int32> Item, ESelectInfo::Type)
																										{ if (Item.IsValid()) FEonformTerrainOutputEditorState::Get().SetOutputResolution(*Item); })
														 [SNew(STextBlock).Text_Lambda([this]()
																													 { return GetResolutionLabel(FEonformTerrainOutputEditorState::Get().GetSettings().OutputResolution); })]] +
								SVerticalBox::Slot().AutoHeight().Padding(0.0f, 2.0f)
										[SNew(SComboBox<TSharedPtr<EEonformTerrainOutputSectionLayout>>)
												 ...
												 .OnGenerateWidget_Lambda([this](TSharedPtr<EEonformTerrainOutputSectionLayout> Item)
																									{ return SNew(STextBlock).Text(GetSectionLayoutLabel(Item.IsValid() ? *Item : EEonformTerrainOutputSectionLayout::Automatic)); })
												 .OnSelectionChanged_Lambda([](TSharedPtr<EEonformTerrainOutputSectionLayout> Item, ESelectInfo::Type)
																										{ if (Item.IsValid()) FEonformTerrainOutputEditorState::Get().SetSectionLayout(*Item); })
														 [SNew(STextBlock).Text_Lambda([this]()
																													 { return GetSectionLayoutLabel(FEonformTerrainOutputEditorState::Get().GetSettings().SectionLayout); })]] +
								SVerticalBox::Slot().AutoHeight().Padding(0.0f, 2.0f)
										[SNew(SBox)
												 .Visibility_Lambda([]()
																						{ return FEonformTerrainOutputEditorState::Get().GetSettings().SectionLayout == EEonformTerrainOutputSectionLayout::Automatic ? EVisibility::Visible : EVisibility::Collapsed; })
														 [SNew(SComboBox<TSharedPtr<EEonformTerrainOutputComplexity>>)
																	...
																	.OnGenerateWidget_Lambda([this](TSharedPtr<EEonformTerrainOutputComplexity> Item)
																													 { return SNew(STextBlock).Text(GetSectionComplexityLabel(Item.IsValid() ? *Item : EEonformTerrainOutputComplexity::Balanced)); })
																	.OnSelectionChanged_Lambda([](TSharedPtr<EEonformTerrainOutputComplexity> Item, ESelectInfo::Type)
																														 { if (Item.IsValid()) FEonformTerrainOutputEditorState::Get().SetSectionComplexity(*Item); })
																			[SNew(STextBlock).Text_Lambda([this]()
																																		{ return GetSectionComplexityLabel(FEonformTerrainOutputEditorState::Get().GetSettings().SectionComplexity); })]]] +
								SVerticalBox::Slot().AutoHeight().Padding(0.0f, 2.0f)
										[SNew(SBox)
												 .Visibility_Lambda([]()
																						{ return FEonformTerrainOutputEditorState::Get().GetSettings().SectionLayout == EEonformTerrainOutputSectionLayout::Explicit ? EVisibility::Visible : EVisibility::Collapsed; })
														 [SNew(SHorizontalBox) + SHorizontalBox::Slot().FillWidth(1.0f).Padding(0.0f, 0.0f, 4.0f, 0.0f)[SNew(SNumericEntryBox<int32>).Label()[SNew(STextBlock).Text(FText::FromString(TEXT("Sections X")))].Value_Lambda([]()
																																																																																																																						 { return FEonformTerrainOutputEditorState::Get().GetSettings().SectionsX; })
																																																																.MinValue(1)
																																																																.OnValueChanged_Lambda([](int32 V)
																																																																											 { FEonformTerrainOutputEditorState::Get().SetSectionsX(V); })] +
															SHorizontalBox::Slot().FillWidth(1.0f).Padding(4.0f, 0.0f, 0.0f, 0.0f)
																	[SNew(SNumericEntryBox<int32>).Label()[SNew(STextBlock).Text(FText::FromString(TEXT("Sections Y")))].Value_Lambda([]()
																																																																										{ return FEonformTerrainOutputEditorState::Get().GetSettings().SectionsY; })
																			 .MinValue(1)
																			 .OnValueChanged_Lambda([](int32 V)
																															{ FEonformTerrainOutputEditorState::Get().SetSectionsY(V); })]]]
