...
namespace
{
	UEonformTerrainGraphAsset* CreateTerrainGraphAssetWithDialog()
	{
		FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>(TEXT("AssetTools"));
		UEonformTerrainGraphAssetFactory* Factory = NewObject<UEonformTerrainGraphAssetFactory>();
		return Cast<UEonformTerrainGraphAsset>(AssetToolsModule.Get().CreateAssetWithDialog(
	...
	UEdGraphPin* ResolveTerrainGraphInputPin(UEonformEditorGraphNode* Node, FName RequestedName)
	{
		if (!Node) return nullptr;
		if (UEdGraphPin* Exact = Node->FindPin(RequestedName, EGPD_Input)) return Exact;

		if (Node->RecipeNodeType == EonformTerrainNodeTypes::Combine)
		{
		...
		else if (Node->RecipeNodeType == EonformTerrainNodeTypes::HydraulicErosion)
		{
	...
	bool TerrainGraphParameterIsVisible(
		const UEonformEditorGraphNode& Node,
		const FEonformTerrainParameterDescriptor& Parameter)
	{
		if (Node.RecipeNodeType == EonformTerrainNodeTypes::PerlinNoise
		...
		if (Node.RecipeNodeType == EonformTerrainNodeTypes::HydraulicErosion
		...
		[
			...
			[
				...
				[
					...
					.OnClicked(this, &SEonformTerrainGraphPanel::NewGraphAsset)
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.Padding(2.0f, 0.0f)
				[
					...
					.OnClicked(this, &SEonformTerrainGraphPanel::OpenGraphAsset)
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.Padding(2.0f, 0.0f)
			...
			[
				SNew(STextBlock)
				.AutoWrapText(true)
				.Text(this, &SEonformTerrainGraphPanel::GetStatusText)
			]
		]
	];
...
TSharedRef<SWidget> SEonformTerrainGraphPanel::CreateGraphEditorWidget()
{
	SGraphEditor::FGraphEditorEvents GraphEvents;
	GraphEvents.OnSelectionChanged = SGraphEditor::FOnSelectionChanged::CreateSP(
		this,
		&SEonformTerrainGraphPanel::OnGraphSelectionChanged);
...
void SEonformTerrainGraphPanel::BuildDefaultRecipeAndGraph()
{
...
void SEonformTerrainGraphPanel::BuildEditorGraphFromRecipe(
	const FEonformTerrainRecipe& Recipe,
	const UEonformTerrainGraphAsset* Asset)
{
	SelectedNode.Reset();
	EditorGraph.Reset(NewObject<UEonformEditorGraph>(GetTransientPackage(), NAME_None, RF_Transient));
	EditorGraph->Schema = UEonformEditorGraphSchema::StaticClass();

	TMap<FGuid, UEonformEditorGraphNode*> NodeMap;
	int32 DefaultIndex = 0;

	for (const FEonformTerrainNode& RecipeNode : Recipe.Nodes)
	{
		UEonformEditorGraphNode* GraphNode = NewObject<UEonformEditorGraphNode>(EditorGraph.Get());
		...
		if (Asset)
		{
			if (const FEonformTerrainNodeLayout* Layout = Asset->FindLayout(RecipeNode.Id))
			{
	...
	{
		UEonformEditorGraphNode* const* FromNode = NodeMap.Find(Connection.FromNode);
		UEonformEditorGraphNode* const* ToNode = NodeMap.Find(Connection.ToNode);
	...
	UEonformEditorGraphNode* TerrainOutputNode = NewObject<UEonformEditorGraphNode>(EditorGraph.Get());
	TerrainOutputNode->Initialize(
		FGuid(0x90909090, 0xA0A0A0A0, 0xB0B0B0B0, 0xC0C0C0C0),
		EonformEditorNodeTypes::TerrainOutput);
	...
	if (UEonformEditorGraphNode* const* RecipeOutputNode = NodeMap.Find(Recipe.OutputNode))
	{
...
bool SEonformTerrainGraphPanel::BuildRecipeFromEditorGraph(
	FEonformTerrainRecipe& OutRecipe,
	FString& OutError) const
{
	OutRecipe = FEonformTerrainRecipe();
	OutError.Reset();
	...
	TArray<UEonformEditorGraphNode*> TerrainNodes;
	UEonformEditorGraphNode* TerrainOutputNode = nullptr;
	for (UEdGraphNode* Node : EditorGraph->Nodes)
	{
		if (UEonformEditorGraphNode* TerrainNode = Cast<UEonformEditorGraphNode>(Node))
		{
			if (TerrainNode->RecipeNodeType == EonformEditorNodeTypes::TerrainOutput)
			{
			...
			FEonformTerrainNode RecipeNode;
	...
	UEonformEditorGraphNode* OutputSourceNode = Cast<UEonformEditorGraphNode>(TerrainOutputInput->LinkedTo[0]->GetOwningNode());
	if (!OutputSourceNode || OutputSourceNode->RecipeNodeType == EonformEditorNodeTypes::TerrainOutput)
	{
	...
	for (UEonformEditorGraphNode* TerrainNode : TerrainNodes)
	{
		for (UEdGraphPin* Pin : TerrainNode->Pins)
		{
			...
			{
				UEonformEditorGraphNode* ToNode = LinkedPin
					? Cast<UEonformEditorGraphNode>(LinkedPin->GetOwningNode())
					: nullptr;
				if (!ToNode || ToNode->RecipeNodeType == EonformEditorNodeTypes::TerrainOutput)
				{
					continue;
				}
...
bool SEonformTerrainGraphPanel::WriteEditorGraphToAsset(
	UEonformTerrainGraphAsset& Asset,
	FString& OutError) const
{
	FEonformTerrainRecipe Recipe;
	...
	for (UEdGraphNode* Node : EditorGraph->Nodes)
	{
		if (const UEonformEditorGraphNode* TerrainNode = Cast<UEonformEditorGraphNode>(Node))
		{
			if (TerrainNode->RecipeNodeType == EonformEditorNodeTypes::TerrainOutput)
			{
				continue;
...
void SEonformTerrainGraphPanel::LoadAsset(UEonformTerrainGraphAsset& Asset)
{
	FString Error;
...
UEonformTerrainGraphAsset* SEonformTerrainGraphPanel::CreateAssetFromCurrentGraph()
{
	UEonformTerrainGraphAsset* Asset = CreateTerrainGraphAssetWithDialog();
	if (!Asset)
	{
		return nullptr;
	}
...
bool SEonformTerrainGraphPanel::SaveCurrentAsset(bool bPromptForCheckout)
{
...
FReply SEonformTerrainGraphPanel::NewGraphAsset()
{
	UEonformTerrainGraphAsset* NewAsset = CreateTerrainGraphAssetWithDialog();
	if (!NewAsset)
	{
...
FReply SEonformTerrainGraphPanel::OpenGraphAsset()
{
	FContentBrowserModule& ContentBrowserModule = FModuleManager::LoadModuleChecked<FContentBrowserModule>(TEXT("ContentBrowser"));

	FOpenAssetDialogConfig Config;
	Config.AssetClassNames.Add(UEonformTerrainGraphAsset::StaticClass()->GetClassPathName());
	Config.bAllowMultipleSelection = false;
	Config.DialogTitleOverride = FText::FromString(TEXT("Open EONFORM Graph"));
	...
	if (UEonformTerrainGraphAsset* Asset = Cast<UEonformTerrainGraphAsset>(SelectedAssets[0].GetAsset()))
	{
		LoadAsset(*Asset);
	}
	else
	{
		StatusText = FText::FromString(TEXT("The selected asset is not a EONFORM graph."));
	}

	return FReply::Handled();
}

FReply SEonformTerrainGraphPanel::SaveGraphAsset()
{
	if (!CurrentAsset.IsValid())
	{
...
void SEonformTerrainGraphPanel::OnGraphSelectionChanged(const TSet<UObject*>& NewSelection)
{
	SelectedNode.Reset();
	if (NewSelection.Num() == 1)
	{
		...
		{
			if (UEonformEditorGraphNode* Node = Cast<UEonformEditorGraphNode>(Object))
			{
				SelectedNode = Node;
			}
		}
...
void SEonformTerrainGraphPanel::RebuildParameterPanel()
{
	if (!ParameterPanel.IsValid()) return;

	ParameterPanel->ClearChildren();
	UEonformEditorGraphNode* Node = SelectedNode.Get();
	if (!Node)
	{
	...
	if (Node->RecipeNodeType == EonformEditorNodeTypes::TerrainOutput)
	{
		ParameterPanel->AddSlot()
		.AutoHeight()
	...
	if (!FEonformTerrainNodeDescriptorRegistry::Get(Node->RecipeNodeType, Descriptor)) return;
	...
	for (const FEonformTerrainParameterDescriptor& Parameter : Descriptor.Parameters)
	{
		...
		{
		case EEonformTerrainParameterType::Number:
			ValueWidget = SNew(SNumericEntryBox<double>)
				...
				{
					if (UEonformEditorGraphNode* Current = WeakNode.Get()) Current->NumericParameters.Add(ParameterName, Value);
				});
			break;

		case EEonformTerrainParameterType::Integer:
			ValueWidget = SNew(SNumericEntryBox<int64>)
				...
				{
					if (UEonformEditorGraphNode* Current = WeakNode.Get()) Current->IntegerParameters.Add(ParameterName, Value);
				});
			break;

		case EEonformTerrainParameterType::Boolean:
			ValueWidget = SNew(SCheckBox)
				...
				{
					if (UEonformEditorGraphNode* Current = WeakNode.Get())
					{
		...
		case EEonformTerrainParameterType::Name:
						...
						for (const FName Option : Options)
						{
							MenuBuilder.AddMenuEntry(
								...
								FUIAction(FExecuteAction::CreateLambda([WeakNode, WeakPanel, ParameterName, Option]()
								{
									...
									if (TSharedPtr<SEonformTerrainGraphPanel> Panel = WeakPanel.Pin())
									{
...
FReply SEonformTerrainGraphPanel::EvaluateGraph()
{
	FEonformTerrainRecipe Recipe;
	FString RecipeError;
	...
	FEonformTerrainEvaluationResult Result = FEonformTerrainEvaluator::Evaluate(Recipe, Context);
	if (!Result.bSuccess)
	{
	...
	const uint64 Revision = FEonformTerrainDatasetRegistry::Publish(
		TEXT("EonformGraph"),
...
FText SEonformTerrainGraphPanel::GetAssetText() const
{
	if (!CurrentAsset.IsValid())
	{
...
FText SEonformTerrainGraphPanel::GetStatusText() const
{
	return StatusText;
}
