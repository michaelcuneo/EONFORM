...
namespace
{
		...
		FString& Error)
	{
		...
		for (int32 Layer = 1; Layer < Values.Num(); ++Layer)
		{
			...
			const FEonformScalarField& LayerField = Values[Layer]->ScalarField;
		...
		FString& Error)
	{
		...
		for (int32 Layer = 1; Layer < Values.Num(); ++Layer)
		{
			...
			const FEonformScalarField* LayerHeight = LayerValue.TerrainDataset.FindScalarField(EonformTerrainFieldNames::Height);
