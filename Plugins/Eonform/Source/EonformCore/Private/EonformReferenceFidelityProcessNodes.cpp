...
namespace EonformReferenceFidelityProcess
{
	...
	{
		FEonformTerrainParameterDescriptor P; P.Name = Name; P.DisplayName = Label; P.Type = EEonformTerrainParameterType::Name; P.DefaultName = Default; for (const FName V : Values) P.NameOptions.Add(V); if (Group) P.Group = Group; return P;
	}

	const FEonformTerrainValue* TerrainInput(const FEonformTerrainNodeInputs& Inputs)
	{
		const FEonformTerrainValue* const* P = Inputs.Find(TEXT("Terrain"));
		const FEonformTerrainValue* V = P ? *P : nullptr;
		return V && V->Type == EEonformTerrainValueType::Terrain && V->IsValid() ? V : nullptr;
	}
	const FEonformScalarField* ScalarInput(const FEonformTerrainNodeInputs& Inputs, FName Name)
	{
	...
	bool BuildSelectiveArea(
		...
		const FEonformTerrainEvaluationContext& Context,
		const FEonformScalarField* ExplicitArea,
		FEonformScalarField& Out)
	{
	...
	bool EvaluateErosion(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs& Inputs, const FEonformTerrainEvaluationContext& Context, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
		const FEonformTerrainValue* Input = TerrainInput(Inputs);
		...
		if (!FEonformHydraulicErosion::Evaluate(
			*Height,
			...
			S,
			R,
			Dataset.FindScalarField(EonformTerrainFieldNames::Rainfall),
	...
	{
		FEonformTerrainNodeDescriptor D; D.Type = EonformTerrainNodeTypes::HydraulicErosion; D.DisplayName = TEXT("Erosion"); D.Category = TEXT("Simulate");
		...
		FEonformTerrainNodeDescriptorRegistry::Register(D); FEonformTerrainNodeRegistry::Register(D.Type, EvaluateErosion);
	}
...
{
	using namespace EonformReferenceFidelityProcess;
	RegisterErosion();
	RegisterErosion2();
}
