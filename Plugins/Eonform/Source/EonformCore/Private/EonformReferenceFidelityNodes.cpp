...
namespace EonformReferenceFidelity
{
		...
		FString& Error)
	{
		...
		if (!Domain.IsValid())
		{
			Error = TEXT("RadialGradient produced an invalid domain.");
			return false;
		}
		...
		for (int32 Y = 0; Y < Domain.Dimensions.Y; ++Y)
		{
			for (int32 X = 0; X < Domain.Dimensions.X; ++X)
			{
				...
				const float T = FMath::Clamp(1.0f - static_cast<float>(Distance), 0.0f, 1.0f);
		...
		{
			Error = TEXT("RadialGradient could not publish Height.");
			return false;
		}
