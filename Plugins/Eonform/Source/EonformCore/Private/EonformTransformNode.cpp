...
namespace
{
	FEonformTerrainPortDescriptor TransformTerrainPort(FName Name, const TCHAR* DisplayName = nullptr)
	{
	...
	FEonformTerrainParameterDescriptor TransformNumberParameter(FName Name, const TCHAR* DisplayName, double DefaultValue, double Minimum, double Maximum, const TCHAR* Group = nullptr)
	{
	...
	FEonformTerrainParameterDescriptor TransformBooleanParameter(FName Name, const TCHAR* DisplayName, bool DefaultValue, const TCHAR* Group = nullptr)
	{
		FEonformTerrainParameterDescriptor Parameter;
		Parameter.Name = Name;
		Parameter.DisplayName = DisplayName;
	...
	FEonformTerrainParameterDescriptor TransformNameParameter(FName Name, const TCHAR* DisplayName, FName DefaultValue, std::initializer_list<FName> Options, const TCHAR* Group = nullptr)
	{
		FEonformTerrainParameterDescriptor Parameter;
		Parameter.Name = Name;
		Parameter.DisplayName = DisplayName;
	...
	float TransformSampleField(const FEonformScalarField& Source, FVector2d SamplePosition, FName Edges, FName Quality)
	{
	...
	bool TransformField(const FEonformTerrainNode& Node, const FEonformScalarField& Source, bool bHeightField, FEonformScalarField& OutField, FString& Error)
	{
		if (!Source.IsValid())
		{
	...
	bool EvaluateTransformNode(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs& Inputs, const FEonformTerrainEvaluationContext&, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
		...
		{
			const FEonformScalarField* SourceField = Input->TerrainDataset.FindScalarField(FieldName);
			if (!SourceField || !SourceField->IsValid())
			{
				Error = FString::Printf(TEXT("Transform input field '%s' is invalid."), *FieldName.ToString());
				return false;
			}
			FEonformScalarField ResultField;
			const bool bHeightField = FieldName == EonformTerrainFieldNames::Height;
...
void RegisterEonformTransformNode()
{
	FEonformTerrainNodeDescriptor Descriptor;
	Descriptor.Type = EonformTerrainNodeTypes::Transform;
	...
	FEonformTerrainNodeRegistry::Register(EonformTerrainNodeTypes::Transform, EvaluateTransformNode);
}
