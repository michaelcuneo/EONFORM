...
namespace
{
	...
	bool EvaluateLinearGradientNode(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs&, const FEonformTerrainEvaluationContext&, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
		...
		FEonformTerrainDataset Dataset; if (!Dataset.SetScalarField(MoveTemp(Field))) { Error = TEXT("LinearGradient could not publish Height."); return false; }
		FEonformTerrainValue Result = FEonformTerrainValue::MakeTerrain(MoveTemp(Dataset), HeightScale); if (!Result.IsValid()) { Error = TEXT("LinearGradient produced invalid terrain."); return false; }
...
{
	FEonformTerrainNodeDescriptor Descriptor; Descriptor.Type = EonformTerrainNodeTypes::LinearGradient; Descriptor.DisplayName = TEXT("LinearGradient"); Descriptor.Category = TEXT("Primitive");
	...
	FEonformTerrainNodeDescriptorRegistry::Register(Descriptor); FEonformTerrainNodeRegistry::Register(EonformTerrainNodeTypes::LinearGradient, EvaluateLinearGradientNode);
}
