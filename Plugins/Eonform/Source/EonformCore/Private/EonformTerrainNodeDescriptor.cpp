...
namespace
{
	...
	TMap<FName, FEonformTerrainNodeDescriptor> DescriptorRegistry;

	FEonformTerrainPortDescriptor TerrainPort(FName Name, const TCHAR* DisplayName = nullptr)
	{
	...
	FEonformTerrainParameterDescriptor NumberParameter(FName Name, const TCHAR* DisplayName, double DefaultValue, double Minimum, double Maximum, const TCHAR* Group = nullptr)
	{
	...
	FEonformTerrainParameterDescriptor IntegerParameter(FName Name, const TCHAR* DisplayName, int64 DefaultValue, int64 Minimum, int64 Maximum, const TCHAR* Group = nullptr)
	{
		FEonformTerrainParameterDescriptor Parameter;
		Parameter.Name = Name;
		Parameter.DisplayName = DisplayName;
	...
	FEonformTerrainParameterDescriptor BooleanParameter(FName Name, const TCHAR* DisplayName, bool DefaultValue, const TCHAR* Group = nullptr)
	{
		FEonformTerrainParameterDescriptor Parameter;
		Parameter.Name = Name;
		Parameter.DisplayName = DisplayName;
	...
	FEonformTerrainParameterDescriptor NameParameter(FName Name, const TCHAR* DisplayName, FName DefaultValue, std::initializer_list<FName> Options, const TCHAR* Group = nullptr)
	{
		FEonformTerrainParameterDescriptor Parameter;
		Parameter.Name = Name;
		Parameter.DisplayName = DisplayName;
...
void FEonformTerrainNodeDescriptorRegistry::Register(const FEonformTerrainNodeDescriptor& Descriptor)
{
...
void FEonformTerrainNodeDescriptorRegistry::RegisterBuiltIns()
{
	FScopeLock Lock(&DescriptorRegistryMutex);

	if (!DescriptorRegistry.Contains(EonformTerrainNodeTypes::SourceDataset))
	{
	...
	if (!DescriptorRegistry.Contains(EonformTerrainNodeTypes::PerlinNoise))
	{
		FEonformTerrainNodeDescriptor Source;
	...
	if (!DescriptorRegistry.Contains(EonformTerrainNodeTypes::TerrainShape))
	{
		FEonformTerrainNodeDescriptor Shape;
	...
	if (!DescriptorRegistry.Contains(EonformTerrainNodeTypes::TerrainContext))
	{
	...
	if (!DescriptorRegistry.Contains(EonformTerrainNodeTypes::Geology))
	{
	...
	if (!DescriptorRegistry.Contains(EonformTerrainNodeTypes::ProcessMasks))
	{
	...
	if (!DescriptorRegistry.Contains(EonformTerrainNodeTypes::Slope))
	{
		FEonformTerrainNodeDescriptor Slope;
	...
	if (!DescriptorRegistry.Contains(EonformTerrainNodeTypes::HydraulicErosion))
	{
...
bool FEonformTerrainNodeDescriptorRegistry::Get(FName NodeType, FEonformTerrainNodeDescriptor& OutDescriptor)
{
	RegisterBuiltIns();
	FScopeLock Lock(&DescriptorRegistryMutex);
	const FEonformTerrainNodeDescriptor* Descriptor = DescriptorRegistry.Find(NodeType);
	if (!Descriptor) return false;
	OutDescriptor = *Descriptor;
	return true;
}

void FEonformTerrainNodeDescriptorRegistry::GetAll(TArray<FEonformTerrainNodeDescriptor>& OutDescriptors)
{
	RegisterBuiltIns();
	FScopeLock Lock(&DescriptorRegistryMutex);
	OutDescriptors.Reset();
	DescriptorRegistry.GenerateValueArray(OutDescriptors);
	OutDescriptors.Sort([](const FEonformTerrainNodeDescriptor& A, const FEonformTerrainNodeDescriptor& B)
	{
...
void FEonformTerrainNodeDescriptorRegistry::Reset()
{
	FScopeLock Lock(&DescriptorRegistryMutex);
