...
namespace
{
	FEonformTerrainPortDescriptor Port(FName Name, const TCHAR* DisplayName, FName DataType)
	{
		FEonformTerrainPortDescriptor Result;
		Result.Name = Name;
	...
	FEonformTerrainParameterDescriptor Number(FName Name, const TCHAR* DisplayName, double Default, double Min, double Max, const TCHAR* Group)
	{
		FEonformTerrainParameterDescriptor Result;
		Result.Name = Name;
		Result.DisplayName = DisplayName;
		Result.Group = Group;
		Result.Type = EEonformTerrainParameterType::Number;
	...
	FEonformTerrainParameterDescriptor Integer(FName Name, const TCHAR* DisplayName, int64 Default, int64 Min, int64 Max, const TCHAR* Group)
	{
		FEonformTerrainParameterDescriptor Result;
		Result.Name = Name;
		Result.DisplayName = DisplayName;
		Result.Group = Group;
		Result.Type = EEonformTerrainParameterType::Integer;
	...
	FEonformTerrainParameterDescriptor Boolean(FName Name, const TCHAR* DisplayName, bool Default, const TCHAR* Group)
	{
		FEonformTerrainParameterDescriptor Result;
		Result.Name = Name;
		Result.DisplayName = DisplayName;
		Result.Group = Group;
		Result.Type = EEonformTerrainParameterType::Boolean;
		Result.DefaultBoolean = Default;
		return Result;
	}

	FEonformTerrainParameterDescriptor Name(FName ParameterName, const TCHAR* DisplayName, FName Default, std::initializer_list<FName> Options, const TCHAR* Group)
	{
		FEonformTerrainParameterDescriptor Result;
		Result.Name = ParameterName;
		Result.DisplayName = DisplayName;
		Result.Group = Group;
		Result.Type = EEonformTerrainParameterType::Name;
		Result.DefaultName = Default;
		for (const FName Option : Options) Result.NameOptions.Add(Option);
		return Result;
	}

	FEonformTerrainParameterDescriptor Color(FName ParameterName, const TCHAR* DisplayName, const FLinearColor& Default, const TCHAR* Group)
	{
		FEonformTerrainParameterDescriptor Result;
		Result.Name = ParameterName;
		Result.DisplayName = DisplayName;
		Result.Group = Group;
		Result.Type = EEonformTerrainParameterType::Color;
		Result.DefaultColor = Default;
		return Result;
	}

	FEonformScalarField MakeField(const FEonformGridDomain& Domain, FName FieldName)
	{
	...
	bool PrepareTerrain(const FEonformTerrainValue& Input, const FEonformTerrainEvaluationContext& Context, FEonformTerrainDataset& Dataset, FString& Error)
	{
		Dataset = Input.TerrainDataset;
		FEonformTerrainDerivedDataSettings Settings;
		if (!FEonformTerrainDerivedData::EnsureContext(Dataset, Input.HeightScale, Context.PhysicalMetrics, &Error)) return false;
		if (!FEonformTerrainDerivedData::EnsureGeology(Dataset, Input.HeightScale, Settings, &Error)) return false;
		if (!FEonformTerrainDerivedData::EnsureProcessMasks(Dataset, Input.HeightScale, Settings, &Error)) return false;
		if (!FEonformTerrainDerivedData::EnsureHydrology(Dataset, Input.HeightScale, Context.PhysicalMetrics, &Error)) return false;
		return true;
	}

	void AutoLevel(FEonformScalarField& Field)
	{
	...
	void Equalize(FEonformScalarField& Field)
	{
		...
		FString& Error)
	{
		...
		FEonformScalarField Texture = MakeField(Slope->Domain, EonformTerrainFieldNames::TextureBase);
		...
		FEonformScalarField Output = Texture;
		...
		Out.Outputs.Add(TEXT("Out"), FEonformTerrainValue::MakeScalarField(MoveTemp(Output)));
		Error.Reset();
		return true;
	}
		...
		FString& Error)
	{
		...
		FEonformScalarField Texture = MakeField(Slope->Domain, EonformTerrainFieldNames::Texturizer);
		for (int32 Y = 0; Y < Slope->Domain.Dimensions.Y; ++Y)
		{
			for (int32 X = 0; X < Slope->Domain.Dimensions.X; ++X)
			{
	...
	bool EvaluateColorThreshold(
		...
		FString& Error)
	{
		const FEonformTerrainValue* const* ColorPtr = Inputs.Find(TEXT("Color"));
		const FEonformTerrainValue* ColorInput = ColorPtr ? *ColorPtr : nullptr;
		if (!ColorInput || ColorInput->Type != EEonformTerrainValueType::Color || !ColorInput->IsValid())
		{
		...
		FEonformScalarField Mask = MakeField(ColorInput->ColorField.Domain, EonformTerrainFieldNames::ColorThreshold);
		...
		Out.Outputs.Add(TEXT("Out"), FEonformTerrainValue::MakeScalarField(Mask));
		if (const FEonformTerrainValue* const* TerrainPtr = Inputs.Find(TEXT("Terrain")))
		{
			const FEonformTerrainValue* Terrain = *TerrainPtr;
			if (Terrain && Terrain->Type == EEonformTerrainValueType::Terrain && Terrain->IsValid())
			{
				FEonformTerrainDataset Dataset = Terrain->TerrainDataset;
				if (!Dataset.SetHeightDerivedScalarField(MoveTemp(Mask)))
				{
					Error = TEXT("ColorThreshold could not publish its mask semantic.");
					return false;
				}
				Out.Outputs.Add(TEXT("Terrain"), FEonformTerrainValue::MakeTerrain(MoveTemp(Dataset), Terrain->HeightScale));
			}
		}
		Error.Reset();
		return true;
...
void RegisterEonformTextureDeriveNodes()
{
	{
		FEonformTerrainNodeDescriptor Descriptor;
		Descriptor.Type = EonformTerrainNodeTypes::TextureBase;
		...
		FEonformTerrainNodeRegistry::Register(Descriptor.Type, EvaluateTextureBase);
	}

	{
		FEonformTerrainNodeDescriptor Descriptor;
		Descriptor.Type = EonformTerrainNodeTypes::Texturizer;
		...
		FEonformTerrainNodeDescriptorRegistry::Register(Descriptor);
		FEonformTerrainNodeRegistry::Register(Descriptor.Type, EvaluateTexturizer);
	}

	{
		FEonformTerrainNodeDescriptor Descriptor;
		Descriptor.Type = EonformTerrainNodeTypes::ColorThreshold;
		...
		FEonformTerrainNodeDescriptorRegistry::Register(Descriptor);
		FEonformTerrainNodeRegistry::Register(Descriptor.Type, EvaluateColorThreshold);
	}
}
