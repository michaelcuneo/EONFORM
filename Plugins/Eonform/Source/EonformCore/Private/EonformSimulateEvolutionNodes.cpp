...
namespace EonformSimulateEvolution
{
	...
	FEonformTerrainPortDescriptor ScalarPort(FName Name, const TCHAR* Label)
	{
		FEonformTerrainPortDescriptor P;
		P.Name = Name;
		P.DisplayName = Label;
	...
	FEonformTerrainParameterDescriptor Range(FName Name, const TCHAR* Label, double DefaultMin, double DefaultMax, double Min, double Max, const TCHAR* Group = nullptr)
	{
		FEonformTerrainParameterDescriptor P;
		P.Name = Name;
		P.DisplayName = Label;
	...
	const FEonformTerrainValue* RequireTerrain(const FEonformTerrainNodeInputs& Inputs, const TCHAR* NodeName, FString& Error)
	{
	...
	void ConfigurePhysicalHydraulic(const FEonformScalarField& Height, float HeightScale, const FEonformTerrainEvaluationContext& Context, FEonformHydraulicErosionSettings& Settings)
	{
		...
		FString& Error)
	{
		...
		if (!FEonformHydraulicErosion::Evaluate(
			*Height,
			...
			Settings,
			Result,
			RainfallMask ? RainfallMask : Dataset.FindScalarField(EonformTerrainFieldNames::Rainfall),
			...
			Dataset.FindScalarField(EonformTerrainFieldNames::SoilDepth)))
		{
	...
	bool EvaluateEasyErosion(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs& Inputs, const FEonformTerrainEvaluationContext& Context, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
		const FEonformTerrainValue* Input = RequireTerrain(Inputs, TEXT("EasyErosion"), Error);
		...
		FEonformTerrainDataset Prepared = Input->TerrainDataset;
		const FEonformScalarField* Height = Prepared.FindScalarField(EonformTerrainFieldNames::Height);
		if (!Height || !Height->IsValid()) { Error = TEXT("EasyErosion input has no valid Height field."); return false; }
		FEonformScalarField Rain = MakeScalar(Height->Domain, TEXT("EasyErosionRain"));
	...
	{
		const FEonformTerrainValue* Input = RequireTerrain(Inputs, TEXT("Erosion2"), Error);
		if (!Input) return false;
		FEonformTerrainDataset Prepared = Input->TerrainDataset;
		const FEonformScalarField* Height = Prepared.FindScalarField(EonformTerrainFieldNames::Height);
		...
		FEonformScalarField Rain = MakeScalar(Height->Domain, TEXT("Erosion2Rain"));
	...
	bool EvaluateThermal2(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs& Inputs, const FEonformTerrainEvaluationContext& Context, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
		const FEonformTerrainValue* Input = RequireTerrain(Inputs, TEXT("Thermal2"), Error);
		if (!Input) return false;
		FEonformTerrainDataset Dataset = Input->TerrainDataset;
		FEonformTerrainDerivedDataSettings Derived;
		if (!FEonformTerrainDerivedData::EnsureHydraulicInputs(Dataset, Input->HeightScale, Context.PhysicalMetrics, Derived, &Error)) return false;
	...
	bool EvaluateCrumble(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs& Inputs, const FEonformTerrainEvaluationContext& Context, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
		const FEonformTerrainValue* Input = RequireTerrain(Inputs, TEXT("Crumble"), Error);
		if (!Input) return false;
		FEonformTerrainDataset Dataset = Input->TerrainDataset;
		FEonformTerrainDerivedDataSettings Derived;
		if (!FEonformTerrainDerivedData::EnsureHydraulicInputs(Dataset, Input->HeightScale, Context.PhysicalMetrics, Derived, &Error)) return false;
		...
		const FEonformScalarField* Source = Dataset.FindScalarField(EonformTerrainFieldNames::Height);
		const FEonformScalarField* Flow = Dataset.FindScalarField(EonformTerrainFieldNames::FlowAccumulation);
		const FEonformScalarField* Deposits = Dataset.FindScalarField(EonformTerrainFieldNames::Deposition);
		...
		const double ElevationMeters = Context.PhysicalMetrics.ResolveElevationScaleMeters(Input->HeightScale);
		FEonformScalarField Height = *Source;
		FEonformScalarField Crumble = MakeScalar(Source->Domain, TEXT("Crumble"));
		...
		FEonformScalarField CrumbleOutput = Crumble;
		Height.Descriptor.Name = EonformTerrainFieldNames::Height;
		Dataset.SetScalarField(MoveTemp(Height));
		Dataset.SetScalarField(Crumble);
		if (!PublishTerrain(MoveTemp(Dataset), Input->HeightScale, Out, Error)) return false;
		Out.Outputs.Add(TEXT("Crumble"), FEonformTerrainValue::MakeScalarField(MoveTemp(CrumbleOutput)));
		return true;
	}

	bool EvaluateHillify(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs& Inputs, const FEonformTerrainEvaluationContext&, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
		const FEonformTerrainValue* Input = RequireTerrain(Inputs, TEXT("Hillify"), Error);
		if (!Input) return false;
		FEonformTerrainDataset Dataset = Input->TerrainDataset;
		const FEonformScalarField* Source = Dataset.FindScalarField(EonformTerrainFieldNames::Height);
		...
		FEonformScalarField Height = *Source;
		FEonformScalarField HillMask = MakeScalar(Source->Domain, TEXT("Hillify"));
		...
		FEonformScalarField HillOutput = HillMask;
		Height.Descriptor.Name = EonformTerrainFieldNames::Height;
		Dataset.SetScalarField(MoveTemp(Height));
		Dataset.SetScalarField(HillMask);
		if (!PublishTerrain(MoveTemp(Dataset), Input->HeightScale, Out, Error)) return false;
		Out.Outputs.Add(TEXT("Mask"), FEonformTerrainValue::MakeScalarField(MoveTemp(HillOutput)));
		return true;
	}
	...
	void RegisterEasyErosion()
	{
		...
		D.Type = EonformTerrainNodeTypes::EasyErosion;
		...
		FEonformTerrainNodeRegistry::Register(D.Type, EvaluateEasyErosion);
	}

	void RegisterErosion2()
	{
		FEonformTerrainNodeDescriptor D;
		D.Type = EonformTerrainNodeTypes::Erosion2;
		...
		FEonformTerrainNodeDescriptorRegistry::Register(D);
		FEonformTerrainNodeRegistry::Register(D.Type, EvaluateErosion2);
	}

	void RegisterThermal2()
	{
		FEonformTerrainNodeDescriptor D;
		D.Type = EonformTerrainNodeTypes::Thermal2;
		...
		FEonformTerrainNodeDescriptorRegistry::Register(D);
		FEonformTerrainNodeRegistry::Register(D.Type, EvaluateThermal2);
	}

	void RegisterCrumble()
	{
		FEonformTerrainNodeDescriptor D;
		D.Type = EonformTerrainNodeTypes::Crumble;
		...
		FEonformTerrainNodeDescriptorRegistry::Register(D);
		FEonformTerrainNodeRegistry::Register(D.Type, EvaluateCrumble);
	}

	void RegisterHillify()
	{
		FEonformTerrainNodeDescriptor D;
		D.Type = EonformTerrainNodeTypes::Hillify;
		...
		FEonformTerrainNodeDescriptorRegistry::Register(D);
		FEonformTerrainNodeRegistry::Register(D.Type, EvaluateHillify);
	}
}

void RegisterEonformSimulateEvolutionNodes()
{
	using namespace EonformSimulateEvolution;
	RegisterEasyErosion();
	RegisterErosion2();
	RegisterThermal2();
	RegisterCrumble();
