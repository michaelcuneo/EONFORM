...
namespace EonformTerrainProceduralOps
{
	namespace
	{
		...
		EonformFastNoiseSIMDCompat::ECellularDistance CellularDistanceType(FName Function)
		{
		...
		{
			const EonformFastNoiseSIMDCompat::ECellularDistance DistanceType = CellularDistanceType(Settings.Function);
			const bool bDistanceReturn = Settings.Form != TEXT("C") && Settings.Form != TEXT("D");
			const EonformFastNoiseSIMDCompat::FCellularSample S = EonformFastNoiseSIMDCompat::Cellular(
			...
			if (Settings.Form == TEXT("N"))
			{
				...
				const EonformFastNoiseSIMDCompat::FCellularSample S1 = EonformFastNoiseSIMDCompat::Cellular(
			...
			float MainFractalBounding)
		{
			...
			{
				EonformFastNoiseSIMDCompat::GradientPerturb(X, Y, Z, Seed, PublicAmplitude, Frequency);
				return;
			}
