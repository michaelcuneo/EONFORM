...
namespace
{
	FEonformScalarField MakeEcologyHeight(const FEonformGridDomain& Domain)
	{
	...
	FEonformTerrainEvaluationContext MakeEcologyContext()
	{
		FEonformTerrainEvaluationContext Context;
		Context.HeightScale = 10000.0f;
		Context.PhysicalMetrics = FEonformTerrainPhysicalMetrics(2400.0, 2400.0, 1200.0, 0.0);
	...
	FEonformTerrainRecipe MakeEcologyRecipe()
	{
		...
		FEonformTerrainNode Trees;
		Trees.Id = FGuid(1002, 2, 2, 2);
		Trees.Type = EonformTerrainNodeTypes::Trees;
		...
		FEonformTerrainNode Shrubs;
		Shrubs.Id = FGuid(1003, 3, 3, 3);
		Shrubs.Type = EonformTerrainNodeTypes::Shrubs;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FEonformEcologySuitabilityTest,
	"Eonform.Core.Graph.EcologySuitability",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FEonformEcologySuitabilityTest::RunTest(const FString& Parameters)
{
	FEonformTerrainNodeDescriptor TreesDescriptor;
	FEonformTerrainNodeDescriptor ShrubsDescriptor;
	TestTrue(TEXT("Trees descriptor exists"), FEonformTerrainNodeDescriptorRegistry::Get(EonformTerrainNodeTypes::Trees, TreesDescriptor));
	TestTrue(TEXT("Shrubs descriptor exists"), FEonformTerrainNodeDescriptorRegistry::Get(EonformTerrainNodeTypes::Shrubs, ShrubsDescriptor));
	...
	const FEonformTerrainEvaluationResult Result = FEonformTerrainEvaluator::Evaluate(MakeEcologyRecipe(), MakeEcologyContext());
	...
	const FEonformScalarField* Trees = Result.Dataset.FindScalarField(TEXT("Trees"));
	const FEonformScalarField* TreeSuitability = Result.Dataset.FindScalarField(TEXT("TreeSuitability"));
	const FEonformScalarField* Shrubs = Result.Dataset.FindScalarField(TEXT("Shrubs"));
	const FEonformScalarField* ShrubSuitability = Result.Dataset.FindScalarField(TEXT("ShrubSuitability"));
	const FEonformScalarField* Moisture = Result.Dataset.FindScalarField(TEXT("VegetationMoisture"));
