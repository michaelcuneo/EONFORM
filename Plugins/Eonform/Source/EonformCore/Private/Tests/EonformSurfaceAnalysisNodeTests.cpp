...
namespace
{
	FEonformScalarField MakeSurfaceAnalysisHeight(const FEonformGridDomain& Domain)
	{
	...
	FEonformTerrainEvaluationContext MakeSurfaceAnalysisContext()
	{
		FEonformTerrainEvaluationContext Context;
		Context.HeightScale = 10000.0f;
		Context.PhysicalMetrics = FEonformTerrainPhysicalMetrics(10000.0, 10000.0, 2200.0, 0.0);
	...
	FEonformTerrainRecipe MakeSingleNodeRecipe(FName Type)
	{
		...
		Derive.Type = Type;
		if (Type == EonformTerrainNodeTypes::Normals)
		{
		...
		else if (Type == EonformTerrainNodeTypes::Occlusion)
		{
	...
	bool ValidateNormalized(FAutomationTestBase& Test, const FEonformScalarField* Field, const TCHAR* Label)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FEonformSurfaceAnalysisDeriveTest,
	"Eonform.Core.Graph.SurfaceAnalysisDerive",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FEonformSurfaceAnalysisDeriveTest::RunTest(const FString& Parameters)
{
	FEonformTerrainNodeDescriptor NormalsDescriptor;
	FEonformTerrainNodeDescriptor OcclusionDescriptor;
	TestTrue(TEXT("Normals descriptor exists"), FEonformTerrainNodeDescriptorRegistry::Get(EonformTerrainNodeTypes::Normals, NormalsDescriptor));
	TestTrue(TEXT("Occlusion descriptor exists"), FEonformTerrainNodeDescriptorRegistry::Get(EonformTerrainNodeTypes::Occlusion, OcclusionDescriptor));
	...
	const FEonformTerrainEvaluationContext Context = MakeSurfaceAnalysisContext();
	const FEonformTerrainEvaluationResult NormalsResult = FEonformTerrainEvaluator::Evaluate(MakeSingleNodeRecipe(EonformTerrainNodeTypes::Normals), Context);
	...
	const FEonformScalarField* NormalX = NormalsResult.Dataset.FindScalarField(EonformTerrainFieldNames::NormalX);
	const FEonformScalarField* NormalY = NormalsResult.Dataset.FindScalarField(EonformTerrainFieldNames::NormalY);
	const FEonformScalarField* NormalZ = NormalsResult.Dataset.FindScalarField(EonformTerrainFieldNames::NormalZ);
	...
	const FEonformTerrainEvaluationResult OcclusionResult = FEonformTerrainEvaluator::Evaluate(MakeSingleNodeRecipe(EonformTerrainNodeTypes::Occlusion), Context);
	...
	const FEonformScalarField* Occlusion = OcclusionResult.Dataset.FindScalarField(EonformTerrainFieldNames::Occlusion);
	...
	VerifyInvalidation(*this, NormalsResult, { EonformTerrainFieldNames::NormalX, EonformTerrainFieldNames::NormalY, EonformTerrainFieldNames::NormalZ });
	VerifyInvalidation(*this, OcclusionResult, { EonformTerrainFieldNames::Occlusion });
	return true;
}
