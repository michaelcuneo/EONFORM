...
namespace EonformSurface
{
	...
	const FEonformTerrainValue* RequireInput(const FEonformTerrainNodeInputs& Inputs, FString& Error)
	{
		...
		if (!Input->TerrainDataset.FindScalarField(EonformTerrainFieldNames::Height))
		{
	...
	bool EvaluateSurface(
		...
		FString& Error)
	{
		const FEonformTerrainValue* Input = RequireInput(Inputs, Error);
		...
		for (int32 Y = 0; Y < H; ++Y)
		{
			...
			for (int32 X = 0; X < W; ++X)
			{
				...
				else if (Node.Type == EonformTerrainNodeTypes::RockNoise)
				{
				...
				else if (Node.Type == EonformTerrainNodeTypes::GroundTexture)
				{
				...
				else if (Node.Type == EonformTerrainNodeTypes::Stratify || Node.Type == EonformTerrainNodeTypes::Sandstone)
				{
					...
					if (Node.Type == EonformTerrainNodeTypes::Sandstone)
					{
				...
				{
					const float Density = FMath::Max(static_cast<float>(Node.GetNumber(TEXT("Density"), Node.Type == EonformTerrainNodeTypes::Stones ? 28.0 : 16.0)), 1.0f);
					float Stamp = 0.0f;
					...
					for (int32 CY = BY - 1; CY <= BY + 1; ++CY)
					{
						for (int32 CX = BX - 1; CX <= BX + 1; ++CX)
						{
							...
							const float Shape = D < 1.0f ? FMath::Pow(1.0f - D * D, Node.Type == EonformTerrainNodeTypes::Stones ? 1.6f : 0.8f) : 0.0f;
	...
	void RegisterOne(FName Type, const TCHAR* Name, const TCHAR* Description, TArray<FEonformTerrainParameterDescriptor> Parameters)
	{
		...
		FEonformTerrainNodeRegistry::Register(Type, EvaluateSurface);
	}
}
...
void RegisterEonformSurfaceNodes()
{
	...
	RegisterOne(EonformTerrainNodeTypes::Bomber, TEXT("Bomber"), TEXT("Scatters procedural surface stamps across terrain."),
	...
	RegisterOne(EonformTerrainNodeTypes::Bulbous, TEXT("Bulbous"), TEXT("Rounds and bulges local terrain forms while preserving macro shape."),
		{ Num(TEXT("Strength"), TEXT("Strength"), 0.5, 0.0, 1.0) });
	RegisterOne(EonformTerrainNodeTypes::Contours, TEXT("Contours"), TEXT("Embosses elevation contour structure onto terrain."),
	...
	RegisterOne(EonformTerrainNodeTypes::Craggy, TEXT("Craggy"), TEXT("Adds slope-aware broken crag and ridge structure."),
	...
	RegisterOne(EonformTerrainNodeTypes::Distress, TEXT("Distress"), TEXT("Chips and weathers exposed terrain with multi-scale damage."),
	...
	RegisterOne(EonformTerrainNodeTypes::Grid, TEXT("Grid"), TEXT("Adds a controllable procedural grid/rib surface pattern."),
		{ Num(TEXT("Strength"), TEXT("Strength"), 0.4, 0.0, 1.0), Num(TEXT("Frequency"), TEXT("Frequency"), 12.0, 1.0, 128.0) });
	RegisterOne(EonformTerrainNodeTypes::GroundTexture, TEXT("GroundTexture"), TEXT("Adds macro and micro ground roughness while preserving landform structure."),
	...
	RegisterOne(EonformTerrainNodeTypes::Outcrops, TEXT("Outcrops"), TEXT("Builds exposed rock protrusions preferentially on steep and convex terrain."),
	...
	RegisterOne(EonformTerrainNodeTypes::Pockmarks, TEXT("Pockmarks"), TEXT("Scatters rounded negative pits over the terrain surface."),
	...
	RegisterOne(EonformTerrainNodeTypes::RockNoise, TEXT("RockNoise"), TEXT("Adds ridged multi-scale noise suitable for competent rock surfaces."),
		{ Num(TEXT("Strength"), TEXT("Strength"), 0.6, 0.0, 2.0), Num(TEXT("Scale"), TEXT("Scale"), 1.0, 0.05, 16.0), Int(TEXT("Seed"), TEXT("Seed"), 1337, -2147483647, 2147483647) });
	RegisterOne(EonformTerrainNodeTypes::Rockscape, TEXT("Rockscape"), TEXT("Combines macro rock breakup and micro rock texture."),
		{ Num(TEXT("Strength"), TEXT("Strength"), 0.7, 0.0, 2.0), Num(TEXT("Scale"), TEXT("Scale"), 1.0, 0.05, 16.0), Int(TEXT("Seed"), TEXT("Seed"), 1337, -2147483647, 2147483647) });
	RegisterOne(EonformTerrainNodeTypes::Roughen, TEXT("Roughen"), TEXT("Adds deterministic high-frequency terrain roughness."),
	...
	RegisterOne(EonformTerrainNodeTypes::Sand, TEXT("Sand"), TEXT("Adds directional dune and ripple structure preferentially on flatter terrain."),
	...
	RegisterOne(EonformTerrainNodeTypes::Sandstone, TEXT("Sandstone"), TEXT("Builds warped sedimentary strata with granular surface breakup."),
	...
	RegisterOne(EonformTerrainNodeTypes::Shatter, TEXT("Shatter"), TEXT("Fractures terrain using cellular crack boundaries."),
		{ Num(TEXT("Strength"), TEXT("Strength"), 0.6, 0.0, 2.0), Num(TEXT("Scale"), TEXT("Scale"), 1.0, 0.05, 16.0), Int(TEXT("Seed"), TEXT("Seed"), 1337, -2147483647, 2147483647) });
	RegisterOne(EonformTerrainNodeTypes::Shear, TEXT("Shear"), TEXT("Shears terrain laterally as a function of elevation."),
	...
	RegisterOne(EonformTerrainNodeTypes::Steps, TEXT("Steps"), TEXT("Quantizes elevation into broad stepped surface levels."),
		{ Num(TEXT("Strength"), TEXT("Strength"), 0.8, 0.0, 1.0), Num(TEXT("Steps"), TEXT("Steps"), 8.0, 2.0, 128.0) });
	RegisterOne(EonformTerrainNodeTypes::Stones, TEXT("Stones"), TEXT("Scatters rounded positive stone forms over terrain."),
	...
	RegisterOne(EonformTerrainNodeTypes::Stratify, TEXT("Stratify"), TEXT("Creates warped elevation strata for layered rock construction."),
		{ Num(TEXT("Strength"), TEXT("Strength"), 0.65, 0.0, 1.0), Num(TEXT("Scale"), TEXT("Scale"), 1.0, 0.05, 16.0), Num(TEXT("Layers"), TEXT("Layers"), 18.0, 2.0, 128.0), Int(TEXT("Seed"), TEXT("Seed"), 1337, -2147483647, 2147483647) });
}
