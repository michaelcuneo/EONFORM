...
namespace
{
	...
	float DenoiseSampleClamped(const FEonformScalarField& Field, int32 X, int32 Y)
	{
	...
	void DenoisePass(const FEonformScalarField& Source, FEonformScalarField& Dest, float Amount, bool bPixelsOnly)
	{
	...
	bool DenoiseProcessField(const FEonformTerrainNode& Node, const FEonformScalarField& Source, FEonformScalarField& OutField, FString& Error)
	{
		if (!Source.IsValid())
		{
	...
	bool EvaluateDenoiseNode(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs& Inputs, const FEonformTerrainEvaluationContext&, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
