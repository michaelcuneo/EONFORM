...
bool FEonformTerrainNode::IsValid() const
{
	return Id.IsValid() && !Type.IsNone();
}

double FEonformTerrainNode::GetNumber(FName Name, double DefaultValue) const
{
	if (const double* Value = NumericParameters.Find(Name)) return *Value;
	return DefaultValue;
}

int64 FEonformTerrainNode::GetInteger(FName Name, int64 DefaultValue) const
{
	if (const int64* Value = IntegerParameters.Find(Name)) return *Value;
	return DefaultValue;
}

bool FEonformTerrainNode::GetBool(FName Name, bool DefaultValue) const
{
	if (const bool* Value = BoolParameters.Find(Name)) return *Value;
	return DefaultValue;
}

FName FEonformTerrainNode::GetName(FName Name, FName DefaultValue) const
{
	if (const FName* Value = NameParameters.Find(Name)) return *Value;
	return DefaultValue;
}

FLinearColor FEonformTerrainNode::GetColor(FName Name, const FLinearColor& DefaultValue) const
{
	if (const FLinearColor* Value = ColorParameters.Find(Name)) return *Value;
	return DefaultValue;
}

bool FEonformTerrainConnection::IsValid() const
{
...
const FEonformTerrainNode* FEonformTerrainRecipe::FindNode(const FGuid& NodeId) const
{
	return Nodes.FindByPredicate([&NodeId](const FEonformTerrainNode& Node) { return Node.Id == NodeId; });
}

bool FEonformTerrainRecipe::Validate(FString* OutError) const
{
	...
	TSet<FGuid> NodeIds;
	for (const FEonformTerrainNode& Node : Nodes)
	{
	...
	for (const FEonformTerrainConnection& Connection : Connections)
	{
...
uint32 FEonformTerrainRecipe::GetDeterministicHash() const
{
	uint32 Hash = GetTypeHash(Version);
	Hash = HashCombineFast(Hash, GetTypeHash(OutputNode));
	TArray<const FEonformTerrainNode*> SortedNodes;
	for (const FEonformTerrainNode& Node : Nodes) SortedNodes.Add(&Node);
	SortedNodes.Sort([](const FEonformTerrainNode& A, const FEonformTerrainNode& B) { return A.Id < B.Id; });
	for (const FEonformTerrainNode* Node : SortedNodes)
	{
	...
	TArray<const FEonformTerrainConnection*> SortedConnections;
	for (const FEonformTerrainConnection& Connection : Connections) SortedConnections.Add(&Connection);
	SortedConnections.Sort([](const FEonformTerrainConnection& A, const FEonformTerrainConnection& B)
	{
	...
	for (const FEonformTerrainConnection* Connection : SortedConnections)
	{
