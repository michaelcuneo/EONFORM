...
namespace
{
	...
	bool EvaluateHemisphereNode(const FEonformTerrainNode& Node, const FEonformTerrainNodeInputs&, const FEonformTerrainEvaluationContext&, FEonformTerrainNodeEvaluation& Out, FString& Error)
	{
		...
		FEonformTerrainDataset Dataset; if (!Dataset.SetScalarField(MoveTemp(Field))) { Error = TEXT("Hemisphere could not publish Height."); return false; }
		FEonformTerrainValue Result = FEonformTerrainValue::MakeTerrain(MoveTemp(Dataset), HeightScale); if (!Result.IsValid()) { Error = TEXT("Hemisphere produced invalid terrain."); return false; }
...
{
	FEonformTerrainNodeDescriptor Descriptor; Descriptor.Type = EonformTerrainNodeTypes::Hemisphere; Descriptor.DisplayName = TEXT("Hemisphere"); Descriptor.Category = TEXT("Primitive");
	...
	FEonformTerrainNodeDescriptorRegistry::Register(Descriptor); FEonformTerrainNodeRegistry::Register(EonformTerrainNodeTypes::Hemisphere, EvaluateHemisphereNode);
}
