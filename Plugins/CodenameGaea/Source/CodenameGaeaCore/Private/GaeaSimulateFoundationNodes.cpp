...
namespace GaeaSimulateFoundation
{
	...
	float LocalSlope01(const FGaeaScalarField& Height, int32 X, int32 Y, double ElevationScaleMeters, const FVector2d& SpacingMeters)
	{
	...
	float LocalConcavity01(const FGaeaScalarField& Height, int32 X, int32 Y)
	{
	...
	const FGaeaScalarField* OptionalScalar(const FGaeaTerrainNodeInputs& Inputs, FName Name, const FGaeaGridDomain& Domain, FString& Error)
	{
		const FGaeaTerrainValue* const* Ptr = Inputs.Find(Name);
		const FGaeaTerrainValue* Value = Ptr ? *Ptr : nullptr;
		if (!Value) return nullptr;
		if (Value->Type != EGaeaTerrainValueType::ScalarField || !Value->ScalarField.IsValid() || Value->ScalarField.Domain != Domain)
		{
		...
		FString& Error)
	{
		...
		const FGaeaScalarField* HeadwatersMask = OptionalScalar(Inputs, TEXT("Headwaters"), Source->Domain, Error);
		...
		FGaeaScalarField River = MakeScalar(Source->Domain, TEXT("River"));
		FGaeaScalarField RiverDepth = MakeScalar(Source->Domain, TEXT("RiverDepth"), EGaeaFieldUnit::Meters);
		TArray<uint8> Active;
		Active.Init(0, W * H);
		TArray<int32> Order;
		Order.Reserve(W * H);
		...
		FGaeaScalarField RiverOutput = River;
		FGaeaScalarField DepthOutput = RiverDepth;
		Height.Descriptor.Name = GaeaTerrainFieldNames::Height;
		Dataset.SetScalarField(MoveTemp(Height));
		...
		if (bRenderSurface)
		{
			FGaeaScalarField WaterSurface = MakeScalar(Source->Domain, TEXT("WaterSurface"), EGaeaFieldUnit::Meters);
		...
		Out.Outputs.Add(TEXT("River"), FGaeaTerrainValue::MakeScalarField(MoveTemp(RiverOutput)));
		...
		FString& Error)
	{
		...
		FGaeaScalarField DepositsOutput = Deposits;
		Height.Descriptor.Name = GaeaTerrainFieldNames::Height;
		Dataset.SetScalarField(MoveTemp(Height));
		...
		bool bScree)
	{
		...
		const double ElevationScaleMeters = Context.PhysicalMetrics.ResolveElevationScaleMeters(Input->HeightScale);
		FGaeaScalarField Height = *Source;
		FGaeaScalarField Mask = MakeScalar(Source->Domain, bScree ? TEXT("Scree") : TEXT("Debris"));
		for (int32 Y = 0; Y < Source->Domain.Dimensions.Y; ++Y)
		{
			for (int32 X = 0; X < Source->Domain.Dimensions.X; ++X)
			{
		...
		FGaeaScalarField MaskOutput = Mask;
		Height.Descriptor.Name = GaeaTerrainFieldNames::Height;
		Dataset.SetScalarField(MoveTemp(Height));
		Dataset.SetScalarField(Mask);
		if (!PublishTerrain(MoveTemp(Dataset), Input->HeightScale, Out, Error)) return false;
		Out.Outputs.Add(bScree ? TEXT("Scree") : TEXT("Debris"), FGaeaTerrainValue::MakeScalarField(MoveTemp(MaskOutput)));
		return true;
	}

	bool EvaluateDebris(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs& Inputs, const FGaeaTerrainEvaluationContext& Context, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
...
void RegisterGaeaSimulateFoundationNodes()
{
	using namespace GaeaSimulateFoundation;

	{
		FGaeaTerrainNodeDescriptor D;
		D.Type = GaeaTerrainNodeTypes::HydroFix;
		...
		FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateHydroFix);
	}

	{
		FGaeaTerrainNodeDescriptor D;
		D.Type = GaeaTerrainNodeTypes::Rivers;
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D);
		FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateRivers);
	}

	{
		FGaeaTerrainNodeDescriptor D;
		D.Type = GaeaTerrainNodeTypes::Sediments;
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D);
		FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateSediments);
	}

	{
		FGaeaTerrainNodeDescriptor D;
		D.Type = GaeaTerrainNodeTypes::Debris;
		...
		D.Parameters.Add(Int(TEXT("Seed"), TEXT("Seed"), 1337, -2147483647, 2147483647));
		FGaeaTerrainNodeDescriptorRegistry::Register(D);
		FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateDebris);
	}

	{
		FGaeaTerrainNodeDescriptor D;
		D.Type = GaeaTerrainNodeTypes::Scree;
		...
		D.Parameters.Add(Int(TEXT("Seed"), TEXT("Seed"), 1337, -2147483647, 2147483647));
		FGaeaTerrainNodeDescriptorRegistry::Register(D);
		FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateScree);
	}
}
