...
namespace
{
	FGaeaTerrainPortDescriptor GaborTerrainPort(FName Name)
	{
		FGaeaTerrainPortDescriptor Port; Port.Name = Name; Port.DisplayName = TEXT("Out"); Port.DataType = TEXT("Terrain"); return Port;
	}
	FGaeaTerrainParameterDescriptor GaborNumber(FName Name, const TCHAR* Label, double Default, double Min, double Max)
	{
	...
	FGaeaTerrainParameterDescriptor GaborInteger(FName Name, const TCHAR* Label, int64 Default)
	{
		FGaeaTerrainParameterDescriptor P; P.Name = Name; P.DisplayName = Label; P.Type = EGaeaTerrainParameterType::Integer; P.DefaultInteger = Default; return P;
	}
	bool EvaluateGaborNode(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs&, const FGaeaTerrainEvaluationContext&, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
		...
		FGaeaTerrainDataset Dataset; if (!Dataset.SetScalarField(MoveTemp(Field))) { Error = TEXT("Gabor could not publish Height."); return false; }
		FGaeaTerrainValue Result = FGaeaTerrainValue::MakeTerrain(MoveTemp(Dataset), HeightScale); if (!Result.IsValid()) { Error = TEXT("Gabor produced invalid terrain."); return false; }
		Out.Outputs.Add(TEXT("Out"), MoveTemp(Result)); return true;
	}
}

void RegisterGaeaGaborNode()
{
	FGaeaTerrainNodeDescriptor Descriptor; Descriptor.Type = GaeaTerrainNodeTypes::Gabor; Descriptor.DisplayName = TEXT("Gabor"); Descriptor.Category = TEXT("Primitive");
	...
	FGaeaTerrainNodeDescriptorRegistry::Register(Descriptor); FGaeaTerrainNodeRegistry::Register(GaeaTerrainNodeTypes::Gabor, EvaluateGaborNode);
}
