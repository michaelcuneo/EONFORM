...
namespace
{
	FGaeaTerrainPortDescriptor HemisphereTerrainPort(FName Name)
	{
		FGaeaTerrainPortDescriptor Port; Port.Name = Name; Port.DisplayName = TEXT("Out"); Port.DataType = TEXT("Terrain"); return Port;
	}
	FGaeaTerrainParameterDescriptor HemisphereNumber(FName Name, const TCHAR* Label, double Default, double Min, double Max)
	{
	...
	bool EvaluateHemisphereNode(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs&, const FGaeaTerrainEvaluationContext&, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
		...
		if (!Domain.IsValid()) { Error = TEXT("Hemisphere produced an invalid grid domain."); return false; }
		...
		for (int32 Y = 0; Y < Resolution; ++Y)
		{
			for (int32 X = 0; X < Resolution; ++X)
			{
				...
				if (Flatten > UE_SMALL_NUMBER && H > Plateau) H = FMath::Lerp(H, Plateau, Flatten);
				Field.AtInterior(X, Y) = FMath::Clamp(H * HeightAmount, -1.0f, 1.0f);
			}
		}
		FGaeaTerrainDataset Dataset; if (!Dataset.SetScalarField(MoveTemp(Field))) { Error = TEXT("Hemisphere could not publish Height."); return false; }
		FGaeaTerrainValue Result = FGaeaTerrainValue::MakeTerrain(MoveTemp(Dataset), HeightScale); if (!Result.IsValid()) { Error = TEXT("Hemisphere produced invalid terrain."); return false; }
		Out.Outputs.Add(TEXT("Out"), MoveTemp(Result)); return true;
	}
}

void RegisterGaeaHemisphereNode()
{
	FGaeaTerrainNodeDescriptor Descriptor; Descriptor.Type = GaeaTerrainNodeTypes::Hemisphere; Descriptor.DisplayName = TEXT("Hemisphere"); Descriptor.Category = TEXT("Primitive");
	Descriptor.Description = TEXT("Generates a hemispherical gradient for domes, mounds, and masks."); Descriptor.Outputs.Add(HemisphereTerrainPort(TEXT("Out")));
	...
	FGaeaTerrainNodeDescriptorRegistry::Register(Descriptor); FGaeaTerrainNodeRegistry::Register(GaeaTerrainNodeTypes::Hemisphere, EvaluateHemisphereNode);
}
