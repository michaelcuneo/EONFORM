...
namespace
{
	...
	FGaeaTerrainParameterDescriptor AngleNodeNumberParameter(
		FName Name,
	...
	{
		float Delta = FMath::Fmod(A - B + 540.0f, 360.0f) - 180.0f;
		...
		FString& Error)
	{
		...
		const float Azimuth = FMath::Fmod(
			FMath::Max(static_cast<float>(Node.GetNumber(TEXT("Azimuth"), 0.0)), 0.0f),
			360.0f);
		const float Minimum = FMath::Clamp(static_cast<float>(Node.GetNumber(TEXT("Min"), 0.0)), 0.0f, 180.0f);
		const float Maximum = FMath::Clamp(static_cast<float>(Node.GetNumber(TEXT("Max"), 45.0)), Minimum, 180.0f);
		...
		for (int32 Y = 0; Y < Dimensions.Y; ++Y)
		{
			for (int32 X = 0; X < Dimensions.X; ++X)
			{
				...
				// Aspect follows the terrain's downhill-facing direction. Azimuth is measured
				// clockwise from +Y (north), matching the conventional compass convention.
...
void RegisterGaeaAngleNode()
{
	...
	Descriptor.Parameters.Add(AngleNodeNumberParameter(TEXT("Min"), TEXT("Min"), 0.0, 0.0, 180.0));
	Descriptor.Parameters.Add(AngleNodeNumberParameter(TEXT("Max"), TEXT("Max"), 45.0, 0.0, 180.0));
