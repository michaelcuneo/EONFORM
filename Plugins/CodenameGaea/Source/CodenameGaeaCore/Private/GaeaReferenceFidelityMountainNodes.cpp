...
namespace GaeaReferenceFidelityMountain
{
	...
	const FGaeaScalarField* AsField(const FGaeaTerrainValue* Value)
	{
		if (!Value || !Value->IsValid()) return nullptr;
		if (Value->Type == EGaeaTerrainValueType::ScalarField) return &Value->ScalarField;
		if (Value->Type == EGaeaTerrainValueType::Terrain) return Value->TerrainDataset.FindScalarField(GaeaTerrainFieldNames::Height);
		return nullptr;
	}

	const FGaeaTerrainValue* RequireTerrain(const FGaeaTerrainNodeInputs& Inputs, FString& Error)
	{
		const FGaeaTerrainValue* Value = Input(Inputs, TEXT("Terrain"));
		if (!Value || Value->Type != EGaeaTerrainValueType::Terrain || !Value->IsValid())
		{
			Error = TEXT("Node requires a valid Terrain input.");
			return nullptr;
		}
		const FGaeaScalarField* Height = Value->TerrainDataset.FindScalarField(GaeaTerrainFieldNames::Height);
	...
	bool PublishTerrain(FGaeaTerrainDataset&& Dataset, float HeightScale, const FGaeaTerrainEvaluationContext& Context, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
	...
	float PhysicalSlopeDegrees(const FGaeaScalarField& Height, int32 X, int32 Y, double ElevationMeters, const FVector2d& Spacing)
	{
	...
	float Concavity01(const FGaeaScalarField& Height, int32 X, int32 Y)
	{
	...
	bool EvaluateThermalCommon(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs& Inputs, const FGaeaTerrainEvaluationContext& Context, FGaeaTerrainNodeEvaluation& Out, FString& Error, bool bClassic)
	{
		const FGaeaTerrainValue* InputTerrain = RequireTerrain(Inputs, Error);
		if (!InputTerrain) return false;
		FGaeaTerrainDataset Dataset = InputTerrain->TerrainDataset;
		FGaeaTerrainDerivedDataSettings Derived;
		if (!FGaeaTerrainDerivedData::EnsureHydraulicInputs(Dataset, InputTerrain->HeightScale, Context.PhysicalMetrics, Derived, &Error)) return false;
	...
	void GradientAt(const FGaeaScalarField& Field, int32 X, int32 Y, int32 Radius, float& GX, float& GY)
	{
	...
	bool EvaluateSlopeWarp(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs& Inputs, const FGaeaTerrainEvaluationContext&, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
		...
		const FGaeaScalarField* GuideInput = AsField(Input(Inputs, TEXT("Guide")));
		if (GuideInput && GuideInput->Domain != Source->Domain) { Error = TEXT("SlopeWarp Guide must share the Input domain."); return false; }
		const FGaeaScalarField& Guide = GuideInput ? *GuideInput : *Source;
	...
	bool EvaluateOutcrops(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs& Inputs, const FGaeaTerrainEvaluationContext&, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
		const FGaeaTerrainValue* InputTerrain = RequireTerrain(Inputs, Error);
		if (!InputTerrain) return false;
		const FGaeaScalarField& Source = *InputTerrain->TerrainDataset.FindScalarField(GaeaTerrainFieldNames::Height);
		FGaeaScalarField Height = Source;
		...
		FGaeaTerrainValue Result = FGaeaTerrainValue::MakeTerrain(MoveTemp(Dataset), InputTerrain->HeightScale);
		Out.Outputs.Add(TEXT("Out"), MoveTemp(Result));
		return true;
	}

	bool EvaluateCraggy(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs& Inputs, const FGaeaTerrainEvaluationContext&, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
		const FGaeaTerrainValue* InputTerrain = RequireTerrain(Inputs, Error);
		if (!InputTerrain) return false;
		const FGaeaScalarField& Source = *InputTerrain->TerrainDataset.FindScalarField(GaeaTerrainFieldNames::Height);
		...
		Out.Outputs.Add(TEXT("Out"), FGaeaTerrainValue::MakeTerrain(MoveTemp(Dataset), InputTerrain->HeightScale));
		return true;
	}

	bool EvaluateSediments(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs& Inputs, const FGaeaTerrainEvaluationContext& Context, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
		const FGaeaTerrainValue* InputTerrain = RequireTerrain(Inputs, Error);
		if (!InputTerrain) return false;
		FGaeaTerrainDataset Dataset = InputTerrain->TerrainDataset;
		const FGaeaScalarField* Source = Dataset.FindScalarField(GaeaTerrainFieldNames::Height);
		FGaeaScalarField Height = *Source;
		FGaeaScalarField Deposits = MakeScalar(Source->Domain, GaeaTerrainFieldNames::Deposits);
		...
		FGaeaScalarField DepositsOut = Deposits;
		Height.Descriptor.Name = GaeaTerrainFieldNames::Height;
		...
		if (!PublishTerrain(MoveTemp(Dataset), InputTerrain->HeightScale, Context, Out, Error)) return false;
		Out.Outputs.Add(TEXT("Deposits"), FGaeaTerrainValue::MakeScalarField(MoveTemp(DepositsOut)));
		return true;
	}

	bool EvaluateScree(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs& Inputs, const FGaeaTerrainEvaluationContext& Context, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
		const FGaeaTerrainValue* InputTerrain = RequireTerrain(Inputs, Error);
		if (!InputTerrain) return false;
		FGaeaTerrainDataset Dataset = InputTerrain->TerrainDataset;
		...
		const FVector2d Spacing = Context.PhysicalMetrics.ResolveSampleSpacingMeters(Source->Domain.Dimensions, Source->Domain.GetCellSize());
		const double ElevationMeters = Context.PhysicalMetrics.ResolveElevationScaleMeters(InputTerrain->HeightScale);
		FGaeaScalarField Suitability = MakeScalar(Source->Domain, TEXT("ScreeSuitability"));
		...
		FGaeaScalarField SpreadField = Suitability;
		if (SpreadRadius > 0)
		{
			for (int32 Y = 0; Y < Source->Domain.Dimensions.Y; ++Y)
			{
		...
		FGaeaScalarField Height = *Source;
		FGaeaScalarField Scree = MakeScalar(Source->Domain, TEXT("Scree"));
		...
		FGaeaScalarField ScreeOut = Scree;
		Height.Descriptor.Name = GaeaTerrainFieldNames::Height;
		...
		if (!PublishTerrain(MoveTemp(Dataset), InputTerrain->HeightScale, Context, Out, Error)) return false;
		Out.Outputs.Add(TEXT("Scree"), FGaeaTerrainValue::MakeScalarField(MoveTemp(ScreeOut)));
		return true;
	}

	void RegisterThermal()
	{
		FGaeaTerrainNodeDescriptor D; D.Type = GaeaTerrainNodeTypes::ThermalErosion; D.DisplayName = TEXT("Thermal"); D.Category = TEXT("Simulate");
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D); FGaeaTerrainNodeRegistry::Register(D.Type, [](const FGaeaTerrainNode& N,const FGaeaTerrainNodeInputs& I,const FGaeaTerrainEvaluationContext& C,FGaeaTerrainNodeEvaluation& O,FString& E){return EvaluateThermalCommon(N,I,C,O,E,true);});
	}

	void RegisterThermal2()
	{
		FGaeaTerrainNodeDescriptor D; D.Type = GaeaTerrainNodeTypes::Thermal2; D.DisplayName = TEXT("Thermal2"); D.Category = TEXT("Simulate");
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D); FGaeaTerrainNodeRegistry::Register(D.Type, [](const FGaeaTerrainNode& N,const FGaeaTerrainNodeInputs& I,const FGaeaTerrainEvaluationContext& C,FGaeaTerrainNodeEvaluation& O,FString& E){return EvaluateThermalCommon(N,I,C,O,E,false);});
	}

	void RegisterSlopeWarp()
	{
		FGaeaTerrainNodeDescriptor D; D.Type = GaeaTerrainNodeTypes::SlopeWarp; D.DisplayName = TEXT("SlopeWarp"); D.Category = TEXT("Modify");
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D); FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateSlopeWarp);
	}

	void RegisterOutcrops()
	{
		FGaeaTerrainNodeDescriptor D; D.Type = GaeaTerrainNodeTypes::Outcrops; D.DisplayName = TEXT("Outcrops"); D.Category = TEXT("Surface"); D.Description = TEXT("Creates rocky outcrops with controllable coverage, variation, strata, breakage, scale, height, and rotation.");
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D); FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateOutcrops);
	}

	void RegisterCraggy()
	{
		FGaeaTerrainNodeDescriptor D; D.Type = GaeaTerrainNodeTypes::Craggy; D.DisplayName = TEXT("Craggy"); D.Category = TEXT("Surface"); D.Description = TEXT("Turns a terrain surface into a craggy, rocky, broken landscape.");
		D.Inputs.Add(Port(TEXT("Terrain"),TEXT("Input"),TEXT("Terrain"))); D.Outputs.Add(Port(TEXT("Out"),TEXT("Out"),TEXT("Terrain")));
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D); FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateCraggy);
	}

	void RegisterSediments()
	{
		FGaeaTerrainNodeDescriptor D; D.Type = GaeaTerrainNodeTypes::Sediments; D.DisplayName = TEXT("Sediments"); D.Category = TEXT("Simulate"); D.Description = TEXT("Creates a thick layer of generic sedimentation over terrain for sediment, sand, or snow workflows.");
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D); FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateSediments);
	}

	void RegisterScree()
	{
		FGaeaTerrainNodeDescriptor D; D.Type = GaeaTerrainNodeTypes::Scree; D.DisplayName = TEXT("Scree"); D.Category = TEXT("Simulate"); D.Description = TEXT("Creates accumulations of loose rock fragments scattered across slopes and terrain edges.");
		...
		FGaeaTerrainNodeDescriptorRegistry::Register(D); FGaeaTerrainNodeRegistry::Register(D.Type, EvaluateScree);
	}
}

void RegisterGaeaReferenceFidelityMountainNodes()
{
	using namespace GaeaReferenceFidelityMountain;
	RegisterThermal();
	RegisterThermal2();
	RegisterSlopeWarp();
	RegisterOutcrops();
