...
void FGaeaScalarField::Initialize(
	const FGaeaGridDomain& InDomain,
	const FGaeaFieldDescriptor& InDescriptor,
	float InitialValue)
{
	Domain = InDomain;
...
bool FGaeaScalarField::IsValid() const
{
	return Domain.IsValid()
		&& Values.Num() == Domain.GetStorageSampleCount();
}

void FGaeaScalarField::Fill(float Value)
{
	if (Values.Num() > 0)
	{
...
float& FGaeaScalarField::AtStorage(int32 X, int32 Y)
{
	check(IsValid());
	return Values[Domain.GetStorageIndex(X, Y)];
}

const float& FGaeaScalarField::AtStorage(int32 X, int32 Y) const
{
	check(IsValid());
	return Values[Domain.GetStorageIndex(X, Y)];
}

float& FGaeaScalarField::AtInterior(int32 X, int32 Y)
{
	check(IsValid());
...
const float& FGaeaScalarField::AtInterior(int32 X, int32 Y) const
{
	check(IsValid());
	check(Domain.IsInteriorCoordinate(X, Y));
	return AtStorage(X + Domain.BorderSamples, Y + Domain.BorderSamples);
...
float FGaeaScalarField::SampleWorld(const FVector2d& WorldPosition, bool bClampToDomain) const
{
	if (!IsValid())
	{
		return 0.0f;
	...
	if (Descriptor.Interpolation == EGaeaInterpolation::Nearest)
	{
