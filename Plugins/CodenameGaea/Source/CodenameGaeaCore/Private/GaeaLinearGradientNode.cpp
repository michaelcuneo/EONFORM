...
namespace
{
	FGaeaTerrainPortDescriptor LinearGradientTerrainPort(FName Name)
	{
		FGaeaTerrainPortDescriptor Port; Port.Name = Name; Port.DisplayName = TEXT("Out"); Port.DataType = TEXT("Terrain"); return Port;
	}
	FGaeaTerrainParameterDescriptor LinearGradientNumber(FName Name, const TCHAR* Label, double Default, double Min, double Max)
	{
	...
	FGaeaTerrainParameterDescriptor LinearGradientName(FName Name, const TCHAR* Label, FName Default, std::initializer_list<FName> Options)
	{
	...
	{
		if (EdgeBehavior == TEXT("Repeat")) return FMath::Frac(V);
	...
	bool EvaluateLinearGradientNode(const FGaeaTerrainNode& Node, const FGaeaTerrainNodeInputs&, const FGaeaTerrainEvaluationContext&, FGaeaTerrainNodeEvaluation& Out, FString& Error)
	{
		...
		if (!Domain.IsValid()) { Error = TEXT("LinearGradient produced an invalid grid domain."); return false; }
		...
		FGaeaTerrainDataset Dataset; if (!Dataset.SetScalarField(MoveTemp(Field))) { Error = TEXT("LinearGradient could not publish Height."); return false; }
		FGaeaTerrainValue Result = FGaeaTerrainValue::MakeTerrain(MoveTemp(Dataset), HeightScale); if (!Result.IsValid()) { Error = TEXT("LinearGradient produced invalid terrain."); return false; }
		Out.Outputs.Add(TEXT("Out"), MoveTemp(Result)); return true;
	}
}

void RegisterGaeaLinearGradientNode()
{
	FGaeaTerrainNodeDescriptor Descriptor; Descriptor.Type = GaeaTerrainNodeTypes::LinearGradient; Descriptor.DisplayName = TEXT("LinearGradient"); Descriptor.Category = TEXT("Primitive");
	Descriptor.Description = TEXT("Generates a straight linear gradient across the terrain."); Descriptor.Outputs.Add(LinearGradientTerrainPort(TEXT("Out")));
	...
	FGaeaTerrainNodeDescriptorRegistry::Register(Descriptor); FGaeaTerrainNodeRegistry::Register(GaeaTerrainNodeTypes::LinearGradient, EvaluateLinearGradientNode);
}
